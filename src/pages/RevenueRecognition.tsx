...
type JeLine = { account: string; debit: number | null; credit: number | null; description: string; group: 'cash' | 'tax' | 'accrual' | 'writeoff' };
...
type Snap = {
  ...
  ar_policy?: { writeoff_after_days: number | null; open_total: number; open_count: number; written_off_total: number; written_off_count: number; bookable_total: number; bookable_count: number; pre_accrual_total: number; pre_accrual_count: number; books_go_live: string; note: string } | null;
  ...
  return (
    <Box>
      ...
      {(() => {
        ...
        if (!je) return null;
        const groupLabel: Record<JeLine['group'], string> = { cash: 'Stripe activity — your existing entry (ties to the Stripe API to the penny)', tax: 'Sales tax by state — backed out of 4000 into the payable accounts', writeoff: 'Bad debt — receivables that passed the collectibility threshold this month', accrual: 'Accrual adjustment — invoice basis (1200 AR ↔ 4000)' };
        const exportCsv = () => {
                ...
                <tbody>
                  {je.lines.map((l, i) => {
                    ...
                    return (
                      <Fragment key={i}>
                        {showGroup && (
                          <tr><Box component="td" colSpan={5} sx={{ ...cell, pt: 1.5, color: l.group === 'accrual' ? 'primary.main' : l.group === 'tax' ? 'warning.main' : l.group === 'writeoff' ? 'error.main' : 'text.secondary', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700, borderBottom: 'none' }}>{groupLabel[l.group]}</Box></tr>
                        )}
                        <tr
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        {snap?.ar_policy?.writeoff_after_days != null && (
          <Alert severity="info" sx={{ mb: 1.5 }}>
            ...
            <br /><br />
            Of that, <strong>{USD0.format(snap.ar_policy.bookable_total)}</strong> ({snap.ar_policy.bookable_count} invoices) is booked to <strong>4950 Uncollectible Income</strong> on the entry for the month it passed {snap.ar_policy.writeoff_after_days} days — only revenue recognized from {monthLabel(snap.ar_policy.books_go_live + '-01')} onward, when the books went accrual. The other <strong>{USD0.format(snap.ar_policy.pre_accrual_total)}</strong> ({snap.ar_policy.pre_accrual_count} invoices) predates that and is <strong>memo only</strong> — those years ran on a cash basis, so no receivable was ever recorded and there is nothing to relieve.<br /><br />These invoices are <strong>still open in Stripe</strong> — this is a reporting policy. Void them or mark them uncollectible in Stripe to make it official.
          </Alert>
        )}
        {isLoading ? <Skeleton variant="rectangular" height={300} /> : (
          <DrillDownPanel<Record<string, unknown>>
