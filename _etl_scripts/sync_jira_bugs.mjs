 ...
 *
 * Output: _etl_scripts/cache/jira_bugs.json
 *
...
const OUT = path.join(ROOT, '_etl_scripts/cache/jira_bugs.json');
...
(async () => {
  ...
  // DEFECTS ONLY, AND DELIBERATELY NOT CUSTOMER-SCOPED.
  // sync_jira_features filters on `cf[10242] IS NOT EMPTY` because the Features page
  // ranks by tagged-customer revenue. That filter is wrong for a bug report: a defect
  // with no customer label is still a defect, and it was making them invisible —
  // DEV-13221 sat in Beau's HubSpot list of open bug tickets and was absent here purely
  // for want of a tag.
  //
  // Types: Bug and Investigate are the two that represent "something is wrong".
  // Customer-reported problems are sometimes triaged onto a Task or New Feature, which
  // is why the page carries a type filter rather than this query being the last word.
  const jql = encodeURIComponent(`project = ${PROJECT} AND issuetype IN (Bug, Investigate) ORDER BY updated DESC`);
  const tickets = [];
  let token = null;
  for (let i = 0; i < 100; i++) {
    let url = `${BASE}/rest/api/3/search/jql?jql=${jql}&maxResults=100&fields=${FIELDS}`;
    if (token) url += `&nextPageToken=${token}`;
    const d = await getJSON(url, 'search DEV bugs');
  ...
  fs.writeFileSync(OUT, JSON.stringify({ fetchedAt: new Date().toISOString(), source: 'jira:DEV issuetype in (Bug, Investigate), all tickets incl. untagged', project: PROJECT, tickets }, null, 2));
  const byStatus = {};
  ...
  console.log(`✓ ${tickets.length} DEV defect tickets (${tagged} distinct customer tags) → ${path.relative(ROOT, OUT)}`);
